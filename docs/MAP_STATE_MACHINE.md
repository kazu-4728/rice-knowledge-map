...
| `picker` | 登録田んぼ一覧 | シート外の地図は自由に移動・拡大縮小可（地図タップは田んぼ詳細を開かない）。中央アイテムのプレビュー時のみ自動パン | 下部シート「登録田んぼ（N件）」、固定ヘッダ／固定「田んぼを登録する」、中央スクロールプレビュー |
...
naming  ──「保存」（新規）─────────────▶ browse（田んぼを地図に追加。選択状態には入らない）
naming  ──「保存」（描き直し）──────────▶ field（更新した田んぼを選択・flyTo）
...
- MapLibre のサイズを DOM 更新後に `resizeMapSoon()` で再計算する
  - `resizeMapSoon` は rAF 1 回のみ（`map.resize()` は `getContainer()` でサイズを自動検出する）
...
2. `window.resize` / `visualViewport.resize`（iOS Safari アドレスバー、PWA viewport 変化）（400ms デバウンス）
3. `returnToBrowse` / `handlePickerSelect` 呼び出し時に `resizeMapSoon`（rAF 1 回）
