...
- 対象: Vercel Cronによる毎日の読み取り専用アクティビティ、Cronエンドポイントの認証、実行成否の最小限のログ、Codexによる定期的な状態確認。
- 非対象: Supabase migration・RLS・Storage・Auth設定、サービスロール鍵の使用、利用者データの書き込み・取得・ログ出力、無制限の再試行、自動課金。
- 進め方:
  1. Cron専用のサーバールートで、既存の公開用Supabaseキーを使った複数の軽量読み取りだけを実行する。取得内容と秘密値は返却・記録しない。
...
- PR単位:
  ...
  - 実装PR: Cronルート、テスト、`vercel.json`。Vercelのproduction環境変数設定とCodexの定期監視は、リポジトリ外の手動作業として実装PRから分離し、それぞれ作成・有効化前にオーナー承認を得る。
