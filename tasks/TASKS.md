...
- 本タスクは2本のPRで進める（通常の「1タスク1Draft PR」原則を、今回はこの2本構成に読み替える）。**PR1完了（PR #80・2026-07-25マージ）。次はPR2に着手する。**

### PR1: 記録運用の改善（完了・PR #80）

分類選択肢8種類化・状態の任意変更＋履歴表示・定点観測導線・家族権限管理画面・写真メタ表示を実装。制約1〜3はいずれもPR #80で解決済み（詳細はPR本文・migration `0009`/`0010`参照）。
...
- Claude Code再開時の伝達文: `mainをpullして、tasks/TASKS.mdの「現在の実行タスク」を読んでください。PR1は完了（PR #80マージ済み）です。PR2（エクスポート機能拡充）に着手してください。制約4（EXIF書き戻し時の撮影情報の正確性）は未決定のため、着手時に「ユーザー確認待ち」の選択肢を提示してオーナー判断を仰いでください。新規依存ライブラリ（ZIP生成・EXIF書き込み）の追加、制約4の対応方針次第では`record_media`へのカラム追加（migration・要承認）が生じます。`
