...
## 0. 次セッション最優先：直近PR機能の作り込み直し（2026-06-13 実機指摘＋静的精査）

ユーザーが本番実機で確認した結果、直近PR(#29/#30)の追加機能は「配線は出来ているが作り込み不足・体感が変わらない」。
コードレベルで致命的クラッシュは無いが、以下を**次セッションで対策する**。ユーザー実機指摘（★）＋静的精査で確認した具体欠陥。

| ID | 優先 | 領域 | 指摘内容 | 根本原因 / 着手点 |
|---|---|---|---|---|
| F-01 | 高 | ★全体UI刷新 | 「全体のUIが何も変わっていない」。コンポーネントは増えたが視覚言語（余白/質感/色/タイポ）が従来のまま。モック基準の本格的デザインパスが必要 | 参照モック画像を基準に HomeScreen/カード/ヘッダーを作り直す。場当たりの shadow/scale 追加では体感が変わらない |
| F-02 | 高 | ★詳細→マップ導線 | 「詳細ページから導線が繋がっていない」。`FieldDetailScreen` の「マップを開く」が `/map`（field 無し）で、どの田んぼか分からない | = T-050。`MapCanvas` に `useSearchParams` で `?field=` を読ませ fitBounds/選択。実装後リンクを `/map?field=` に戻し「マップで確認」へ |
| F-03 | 高 | ★戻るボタン未整備 | 「全ページに戻るボタンが無い」。`/calendar`・`/fields`(一覧)・`/records`(一覧) 等に未設置（タブ系だが遷移元から戻れない） | 各 page.tsx の AppShell に `backDynamic` or `backHref` を付与。要件整理（タブ直下は不要か、サブ画面は必須か）して統一 |
| F-04 | 高 | ★画像読み込みが遅い | 「画像読み込みが遅くなっている」 | 要因候補: 全画像に Ken Burns（再描画負荷）/ signed URL を1件ずつ生成 / 生 `<img>` で最適化なし / アップロード画像が大きい。プロファイルして特定。`createSignedUrls` のバッチ化・遅延読み込み・サイズ指定・Ken Burns の適用範囲縮小を検討 |
| F-05 | 中 | ★スプラッシュ演出 | 「アニメが地味/変わって見えない」。コードは動作するが体感が弱い | モーションを抜本的に強化（ズーム量/フェード/テキスト演出）。実機GIFで確認。併せて F-09 デッドコード整理 |
| F-06 | 中 | ★カレンダー | 月移動で選択日がリセットされず、別月の選択日に空リストが出て混乱（`CalendarScreen.tsx:28,107-121`） | 月変更時に `setSelectedDate(null)`。カレンダー全体のUX/見た目も F-01 と合わせて見直し |
| F-07 | 中 | 天気ヘッダー | 位置情報拒否/API失敗で「天気不明」表示＝実機で天気が出ず壊れて見える恐れ。毎起動で位置情報要求 | 実機で実通信を確認。失敗時フォールバック表示の改善、位置情報要求タイミングの最適化（初回のみ/明示ボタン） |
| F-08 | 低 | フィールドID | `id: String(ft.id ?? ft.properties?.id ?? "")` で空ID化するエッジケース未処理（calendar/export/home） | 空IDを除外 or ロード時バリデーション |
| F-09 | 低 | デッドコード | `HeroSection.tsx`/`AppIntroSection.tsx` が未import（未定義 `animate-hero-zoom` 参照）。「AIが概要を自動作成」文言もここにあり実際は非表示 | 削除。`useSpeechRecognition` の未使用 `interim` も整理 |

**進め方の注意:** F-01（UI刷新）はユーザーの最大の不満。場当たり修正でなくモック準拠でまとまった改修を。UI作業は必ず実機/PreviewURLで体感確認してから完了扱いにする。
...
### 2026-06-13（続き4） — 直近PR機能の実態精査（実装なし・調査のみ）

**背景:** ユーザーが「最新PRの機能追加が上手くいっていない／プラン内容に近づいていない」と指摘。コード全体を3領域に分けて静的精査した。

**精査結果サマリ:**
- **コア（マップ/記録）は実際に機能している**: 地理院タイル・なぞり描き登録・田んぼ編集・3色ピンCRUD・常設ボトムシート・写真/音声記録・保存前確認・記録詳細・コメント・「対応済みにする」すべて Supabase に配線済みで動作。`?field=`/`?point=` 絞り込みも動く。
- **直近PR(#29/#30)の機能も配線レベルでは動作**: 天気(JMA API+GPS)・カレンダーCRUD・FAB・トースト・PDFエクスポート・音声入力6箇所・スプラッシュ演出。致命的クラッシュは無し。
- **ただし作り込み不足・具体欠陥あり** → 上の「§0 次セッション最優先」に F-01〜F-09 として整理。
- **デッドコード判明**: `HeroSection.tsx`/`AppIntroSection.tsx` は未importの取り残し。前者は未定義 `animate-hero-zoom` を参照、後者に「AIが概要を自動作成します」の文言があるが**実際は非表示**（＝AI未実装だがユーザーには見えていない）。
- **AI整理(T-048)** は未実装のまま（ビジョン §2 に記載あるが任意機能扱い）。

**実機指摘（ユーザー）:** スプラッシュ演出が地味 / カレンダー挙動 / 詳細→マップ導線が繋がらない / 全体UI刷新が変わって見えない / 戻るボタン未整備 / 画像読み込みが遅い。→ §0 に反映。

**この回はコード変更なし。次セッションで §0 に着手する。**
