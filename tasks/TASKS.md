...
### UIUX刷新 第2単位: 残りページの統一モック作成

- 状態: `IN_PROGRESS`
- 目的: PR #102で確立した「写真を主役に、固定知識と変動記録を混在させない」体験を、未刷新ページへ展開する前に全体像とPR単位を確定する。
- 対象: ランディング、カレンダー、記録タイムライン、マップ全体、メニュー・使い方ガイドの現状確認と、スマホ向け統一モックの作成。
- 非対象: モック承認前の実装、Supabase migration・RLS・Storage・認証・本番設定、LINE取り込み、AI自動整理。
- 進め方:
  1. 対象ページをブラウザで確認し、情報の重複、導線の混在、写真が活かされていない箇所を整理する。
  2. PR #102のデザイン言語と矛盾しない統一モックを作成し、画面表示してオーナー承認を得る。
  3. 承認後、利用体験ごとの実装範囲・非対象・受入条件・PR単位を本項目へ追記してから実装する。
- 完了条件: 対象ページの統一モック、ページ間導線、実装順序、各PRの明確な完了点がオーナー承認済みであること。現時点では実装完了を意味しない。
- 次回の開始点: 対象5領域の現状をブラウザとコードで確認し、まず統一モックを作成する。追加の着手確認は不要だが、実装はモック承認まで開始しない。
