...
| T-059 | IN_PROGRESS | **UI/UX 大規模改修**。Step1-3はPR #51/#52で完了したが「ほとんど変わっていない」と却下→ **全面再構成プラン「田んぼOS」承認済み（2026-07-02作業ログ参照）**。PR-1〜4で実装中。従来のStep3以降の残タスクはこの新プランに吸収 |
...
### 2026-07-02 — 全面再構成プラン「田んぼOS」承認・実装開始（T-059の新方針）

前回提示のPR-A〜D案は「既存サイトの小変更に見える」として却下。**開いた瞬間から違うアプリに見える**ことを目標に、以下の「田んぼOS」プランを再提案し**ユーザー承認済み**（プラン全文はセッションのプランファイル。要点のみ記載）:

**コンセプト**: 記録ツールではなく「開くたびに田んぼの"今"が語りかけてくるOS」。Googleマップ（空間）× LINE（会話）× アグリノート（管理）+ Instagramストーリー（没入）+ 稲作暦（季節文脈）。

**6つの変革レイヤー:**
1. オープニング「今日の田んぼ」ストーリー（1日1回・フルスクリーンカード→地図に着地。天気/農事暦/未対応/家族の記録で構成）
2. ダーク管制室マップ（`/map` のヘッダー廃止・フローティング検索ピル・ダークガラスUI・ポリゴン信号色発光・スナップ式シート）
3. 音声トランシーバーFAB（長押し→即録音→GPS田んぼ自動判定→投稿。既存MediaRecorder/GPS判定/recordSaveの組み替え）
4. 農事暦シーズンエンジン（`src/lib/season.ts`。フェーズ推定・シーズンバー・フェーズ連動テーマ/ヒント。localStorage補正・DB変更なし）
5. 定点観測タイムマシン（田んぼ詳細に月別写真スライダー。前年比較の器）
6. 「次にやること」ガイドUX（ネクストアクションカード常設・ラベル付きFAB展開メニュー・空状態=できることリスト・初回コーチマーク）

**ユーザーフィードバックで確定した追加方針:**
- トークは**家族の統合トークルーム1本**が主役（全田んぼの記録・コメントが1つのタイムライン。メッセージに田んぼチップ、チップタップで田んぼ別ビューに絞り込み）。「どこにコメントがあるか分からない」問題を解消
- 「次に何をすべきか・何ができるか」を常に明示するUX（レイヤー6）
- **画像はリポジトリに置かない**（GitHubを重くしない）。デザイン用実写は Supabase Storage 公開バケット `app-assets`（作成前にユーザー承認）から配信。リポジトリ内は軽量SVGフォールバックのみ

**PR分割**: PR-1 第一印象の反転（基盤+ダークマップ+ストーリー+ガイドUX+ナビ4系統）／PR-2 トーク空間（統合ルーム+トランシーバー）／PR-3 管理空間+シーズン（ダッシュボード+農事暦+定点観測）／PR-4 仕上げ。

**制約**: MapCanvas内部は触らない（スタイル定数のみ）・DBスキーマ変更なし・モバイル優先（PC対応は後で必ず）・BottomTabBar再導入しない。技術追加: `motion` / `embla-carousel-react` / shadcn追加コンポーネント。
