...
- 状態: `IN_PROGRESS`
- 目的: 無料枠のSupabaseプロジェクトが、利用のない週でも自動停止しにくい状態を保つ。停止した場合も早期に検知できる運用にする。
- 対象: Vercel Cronによる毎日の読み取り専用アクティビティ、Cronエンドポイントの認証、稼働維持専用RPCを追加するSupabase migration、実行成否の最小限のログ、Codexによる定期的な状態確認。
- 非対象: 稼働維持専用RPC以外のSupabase migration・RLS・Storage・Auth設定、サービスロール鍵の使用、利用者データの書き込み・取得・ログ出力、無制限の再試行、自動課金。
- 進め方:
  1. Cron専用のサーバールートで、既存の公開用Supabaseキーを使って、利用者データを取得しない稼働維持専用RPCを固定回数だけ実行する。RPCはmigrationで追加し、取得内容と秘密値は返却・記録しない。
  2. Vercelの本番環境変数 `CRON_SECRET` をリポジトリ外に設定し、正しい認証ヘッダーがない呼び出しは即時に拒否する。
  3. Vercel Hobbyの上限内で、1日1回だけ実行する。ループ・バックオフ・自己呼び出しは実装しない。
  4. Codexの定期確認でプロジェクト状態を確認し、停止・失敗時だけ通知する。自動復旧は状態を確認してから一度だけ行う。
- 受入条件:
  - 未認証のHTTPリクエストはDBへ到達せず401となる。
  - 認証済みのCron呼び出しは、書き込みを伴わない複数のDBリクエストを1回だけ実行し、利用者データ・秘密値を返さない。
  - Vercelのproductionで日次Cronが登録され、ソース・ログ・PRに秘密値が含まれない。
  - 型検査、Lint、build、認証拒否と認証済み成功の自動テストが成功する。
- PR単位:
  - 文書PR: このタスクと、後続UIUXモック作業を分離して記録する。
  - 実装PR: Cronルート、稼働維持専用RPC migration、テスト、`vercel.json`。Vercelのproduction環境変数設定とCodexの定期監視は、リポジトリ外の手動作業として実装PRから分離し、それぞれ作成・有効化前にオーナー承認を得る。
  - オーナー承認例外（2026-08-29）: PRの過剰分割を避けるため、PR #105に限り、レビューで判明した稼働維持専用RPC migrationの範囲修正を実装と同じPRに含める。別の文書PRは作成しない。
- 完了条件: 実装PRがマージされ、productionのCronが安全に1回成功し、定期監視が有効であること。
- 次回の開始点: PR #105のCloud Codex再レビュー結果を確認し、承認後に各指摘へ検証結果を返信してResolveする。
...
- UIUX刷新 第2単位: 残りページの統一モック作成（ランディング、カレンダー、記録タイムライン、マップ全体、メニュー・使い方ガイド）。PR #102で確立した「写真を主役に、固定知識と変動記録を混在させない」体験を未刷新ページへ展開する前に、現状確認とスマホ向け統一モックを作成する。モック承認前の実装、Supabase migration・RLS・Storage・認証・本番設定、LINE取り込み、AI自動整理は対象外。
