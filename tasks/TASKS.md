...
| ID | 状態 | 内容 |
|---|---|---|
| PR-E | IN_PROGRESS | ヒーロー演出強化（Ken Burns・進捗バー・時間差フェード）・折りたたみ修正（使い方=全表示/最近の記録=5件） |
| PR-F | TODO | 全ページ戻るボタン・UIアプリ感向上（FAB・トースト・押下フィードバック・空状態挿絵・季節テーマ） |
| PR-G | TODO | ヘッダー天気予報（現在地GPS→気象庁API・1週間インタラクティブ表示） |
| PR-H | TODO | カレンダー家族共有（migration 0005・月表示・作業予定登録・/calendar 新設） |
| PR-I | TODO | 未対応異常ダッシュボード・家族活動フィード・年次比較・共有/PDFエクスポート |
