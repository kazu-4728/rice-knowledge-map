...
UI/UX 改善フェーズ。マップUI・操作モデル・iOS対応は実装完了（PR #38〜#40）。

**次に着手するタスク（優先順）:**

1. `/map` 通常閲覧の密度・導線改善（UX-071）— 実機課題が 1 件あれば着手可
2. 初回利用導線（UX-070）— 未ログイン・田んぼ0件・登録済みの3状態を整理
3. 田んぼ詳細の CTA を「記録する・地点追加・履歴を見る」に絞る（UX-072）
4. マップと記録の往復導線（UX-073）— 記録詳細から「マップで見る」を追加
5. マップ以外の画面（/home /fields /records）の構造整理
