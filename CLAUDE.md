...
## 1. 現在の状態（2026-06-14 更新）

- **最新の進捗・残タスク・詳細な作業ログは `tasks/TASKS.md` が最新の一次情報**。このセクションは要約。
- **直近（2026-06-14）**: PR #31（`8bde66d`・squashマージ済み）で、トップ`/`を王道Webランディング（ヒーロー＋機能＋CTA・大胆Ken Burns＋スクロールパララックス）に刷新。Codex/セルフレビュー指摘14点に対応（「未対応」判定を pointType ベースに統一＝既定 status=open の誤判定と旧データ取りこぼしを是正、export/田んぼ詳細/未対応導線を全件ページング取得、`RemotePhoto` キャッシュ透明化修正 ほか）。**カレンダーは設計判断で「単一アクティブグループ（ensureGroupId）」に統一**し viewer 書込みを非表示＋0行RLS拒否を検出。ホーム/記録一覧は従来どおり全グループ横断。**複数グループの本格対応は将来タスクで保留**。
- **次の最優先はユーザーの本番実機確認**（TASKS.md の U-002/U-005）。デザインは文章でなく実機/プレビューで見せて判断してもらう。
